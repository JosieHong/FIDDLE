...
def combine_spectra(spec1_mz, spec1_intensity, spec2_mz, spec2_intensity, alpha): 
	...
	chim_spec_intensity = np.concatenate([alpha * spec1_intensity, (1 - alpha) * spec2_intensity])
...
def extract_ce(spectrum, encoder): 
	...
	ce, _ = parse_collision_energy(ce_str=spectrum['params']['collision_energy'], 
...
def make_chimeric_dataset(data, alpha, encoder, mz_tol=0.5, num_pairs=1000, seed=42):
	...
	all_entries = [(d, bin_mz(float(d['params']['precursor_mz']), bin_size=mz_tol/2), extract_ce(d, encoder), d['params']['precursor_type']) for d in data]

	for _ in tqdm(range(num_pairs), desc=f"Generating chimeric {int(alpha * 100)}:{int((1-alpha)*100)}"):
		...
		chim_spec_mz, chim_spec_intensity = combine_spectra(entry_a['m/z array'], entry_a['intensity array'], entry_b['m/z array'], entry_b['intensity array'], alpha)

		chim_spec = copy.deepcopy(entry_a)
		chim_spec['m/z array'] = chim_spec_mz
		chim_spec['intensity array'] = chim_spec_intensity * 1000  # Scale intensity to match original
		chim_spec['params']['title'] = f"{entry_a['params']['title']}_{entry_b['params']['title']}_chimera_{int(alpha*100)}"
	...
	print(f"Generated {len(chimeric_data)} chimeric spectra with ratio {int(alpha * 100)}:{int((1 - alpha) * 100)}")
...
if __name__ == "__main__":
	...
	parser.add_argument('--out_dir', type=str, default='./data/chimeric/',
						help='Directory to save individual chimeric spectra files')
	parser.add_argument('--mix_ratios', type=int, nargs='+', default=[90, 70, 50],
	...
	for ratio in args.mix_ratios:
		alpha = ratio / 100.0
		chimeric_data = make_chimeric_dataset(data, alpha, encoder, mz_tol=args.mz_tol, num_pairs=args.num_pairs)

		out_path = os.path.join(args.out_dir, "{}_chimeric{}.mgf".format(args.mgf_data.split('/')[-1].replace('.mgf', ''), ratio))

		mgf.write(chimeric_data, out_path, key_order=mgf_key_order, file_mode="w")

		print(f"Saved {len(chimeric_data)} spectra for ratio {ratio}:{100-ratio} to {out_path}")
