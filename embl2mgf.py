...
def parse_input_text(text):
	...
	if collision_energy_match:
		...
		if ',' in collision_energy:
			collision_energy = np.mean([float(ce) for ce in collision_energy.split(',')]) # average collision energy
...
def generate_mgf_block(data, type2charge):
	...
	assert 'id' in data and \
	   'precursor_type' in data and \
	   'formula' in data and \
	   'mz_array' in data and \
	   'intensity_array' in data, 'Some required fields are missing in the input data'
	...
	assert precursor_type in type2charge, 'Unknown precursor type: {}'.format(precursor_type)
...
if __name__ == "__main__": 
	...
	parser.add_argument('--data_config_path', type=str, default='./config/fiddle_tcn_embl.yml',
	...
	for input_item in input_text.split("$$$$"): 
		...
		if parsed_item is None or parsed_item['precursor_type'] not in type2charge.keys(): 
			continue
		# Generate MGF block
	...
	print('Filtered out {} invalid spectra'.format(len(spectra) - len(filtered_spectra)))
