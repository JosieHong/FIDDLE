...
from utils import sdf2mgf, filter_spec, precursor_mz_calculator, mgf_key_order, monoisotopic_mass_calculator
...
if __name__ == "__main__": 
	...
	parser.add_argument('--data_config_path', type=str, default='./config/fiddle_tcn_casmi.yml',
						help='path to configuration')
	args = parser.parse_args()

	assert os.path.exists(os.path.join(args.raw_dir, 'casmi2016', 'MoNA-export-CASMI_2016.sdf'))

	assert os.path.exists(os.path.join(args.raw_dir, 'casmi2017', 'Chal1to45Summary.csv'))
	assert os.path.exists(os.path.join(args.raw_dir, 'casmi2017', 'CASMI-solutions.csv'))
	assert os.path.exists(os.path.join(args.raw_dir, 'casmi2017', 'challenges-001-045-msms-mgf-20170908/'))
	assert os.path.exists(os.path.join(args.raw_dir, 'casmi2017', 'challenges-001-045-ms-mgf-20170908/'))
	...
	casmi16_spectra = sdf2mgf(path=os.path.join(args.raw_dir, 'casmi2016', 'MoNA-export-CASMI_2016.sdf'), prefix='casmi16')

	print('Convert CASMI2017...')
	df_sum1 = pd.read_csv(os.path.join(args.raw_dir, 'casmi2017', 'Chal1to45Summary.csv'))
	df_sum1['challengename'] = df_sum1['MS_files'].apply(lambda x: x.rstrip('-msms.txt'))
	df_sum2 = pd.read_csv(os.path.join(args.raw_dir, 'casmi2017', 'challenges-001-045-msms-mgf-20170908/', 'summary-001-045.csv'), sep='\t')
	df_sol = pd.read_csv(os.path.join(args.raw_dir, 'casmi2017', 'CASMI-solutions.csv'), index_col=0)
	df_sol['SMILES'] = df_sol['SMILES'].apply(lambda x: x.rstrip('\t'))
	df_sum = pd.merge(df_sum1, df_sum2, left_on='challengename', right_on='challengename')
	...
	for idx, row in tqdm(df_sum.iterrows(), total=df_sum.shape[0]): 
		...
		msms_mgf_file = os.path.join(args.raw_dir, 'casmi2017', 'challenges-001-045-msms-mgf-20170908', msms_file_name)
		...
		ms_mgf_file = os.path.join(args.raw_dir, 'casmi2017', 'challenges-001-045-ms-mgf-20170908', ms_file_name)
		...
		with mgf.read(msms_mgf_file) as reader: 
			...
			spectrum['params']['molmass'] = monoisotopic_mass_calculator(Chem.MolFromSmiles(smiles), mode='mol')
			spectrum['params']['ms_level'] = 'MS2'
			spectrum['params']['ionmode'] = ion_mode
			spectrum['params']['source_instrument'] = org_spectrum['params']['source_instrument']
			spectrum['params']['instrument_type'] = org_spectrum['params']['source_instrument']
	...
	casmi16_spectra, casmi_smiles_list = filter_spec(casmi16_spectra, config['casmi16'], type2charge=config['encoding']['type2charge'])
	# calculate ppm
	for idx, spec in enumerate(casmi16_spectra): 
		ppm = cal_ppm(spec['params']['molmass'], spec['params']['precursor_type'], spec['params']['precursor_mz']) # charge 1
		casmi16_spectra[idx]['params']['ppm'] = ppm
	print('Get {} spectra from CASMI2016'.format(len(casmi_smiles_list)))
	mgf.write(casmi16_spectra, os.path.join(args.mgf_dir, 'casmi2016.mgf'), key_order=mgf_key_order+['ppm'], file_mode="w")

	print('Filter CASMI2017 spectra...')
	casmi17_spectra, casmi_smiles_list = filter_spec(casmi17_spectra, config['casmi17'], type2charge=config['encoding']['type2charge'])
	# calculate ppm
	...
	mgf.write(casmi17_spectra, os.path.join(args.mgf_dir, 'casmi2017.mgf'), key_order=mgf_key_order+['ppm'], file_mode="w")
