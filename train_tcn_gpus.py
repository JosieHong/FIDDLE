...
def train_step(model, loader, 
	...
	with tqdm(total=len(loader)) as bar: 
		for step, batch in enumerate(loader): 
			...
			mae = mae + torch.mean(torch.abs(y - pred_f), dim=1).tolist() # Josie: for debug
			mass_mae = mass_mae + torch.abs(mass - pred_mass).tolist() # Josie: for debug
			atomnum_mae = atomnum_mae + torch.abs(atomnum - pred_atomnum).tolist() # Josie: for debug
			hcnum_mae = hcnum_mae + torch.abs(hcnum - pred_hcnum).tolist() # Josie: for debug
...
if __name__ == "__main__":
	...
	# 1. Data
	train_set = MS2FDataset(args.train_data, 
	...
	plateau_scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.5, patience=5)
	...
	for epoch in range(epoch_start+1, config['train']['epochs']+1): 
		...
		train_mae, train_mass_mae, train_atomnum_mae, train_hcnum_mae, train_loss_ad, train_loss = train_step(model, 
																		train_loader, 
																		optimizer, 
